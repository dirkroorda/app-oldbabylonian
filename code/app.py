...
class TfApp(object):
    ...
    def _pretty(
        ...
        if isBigType:
            ...
            children = tuple(
                c for c in L.d(n) if F.otype.v(c) == WORD or F.type.v(c) == COMMENTLINE
            )
        elif nType == WORD:
        ...
        if nType == DOCUMENT:
            ...
            featurePart = getFeatures(app, n, ("object",), **options,)
        elif nType == LINE:
            ...
            heading = text if isHtml else htmlEsc(text)
            featurePart = getFeatures(app, n, SIGN_FEATURES, withName=True, **options,)
